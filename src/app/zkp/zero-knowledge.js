/*
The following code converts the login cridentials to zero knowledge proof while
authenticating the user onto the page Author :Gaurav Savanur Project:Zero
Knowledge Proofs within Inventory Supplychains
*/
      ...
      if (err) {
        ...
        cache.get(username, { asBuffer: false }, function (err, cacheJson) {
          if (err) {
            ...
            // we now need to load the challenge data from the cache to check
            // the credentials {A,M1}
            const newPrivate = JSON.parse(cacheJson);
            server = new SRP6JavascriptServerSession();
            server.fromPrivateStoreState(newPrivate);

            // the server takes `A`, internally computes `M1` based on the
            // verifier, and checks that its `M1` matches the value sent from
            // the client. If not it throws an exception. If the `M1` match then
            // the password proof is valid. It then generates `M2` which is a
            // proof that the server has the shared session key.
            try {
